...
class Icon(Icon):
    def update_for_lives(self) -> None:
        ...
        Note: base Icon stores player as a weakref; dereference before use."""
        try:
            player = self._player() if self._player is not None else None
        except Exception:
            player = None
...
class PowBox(Bomb):
    ...
    def __init__(self,
        ...
        # API 9 Bomb requires keyword args; pass explicitly to avoid TypeError.
        Bomb.__init__(self,
                      position=position,
                      velocity=velocity,
                      bomb_type='tnt',
...
class SuperSmash(bs.TeamGameActivity[Player, Team]):
    ...
    def get_available_settings(
        ...
        settings = [
            ...
            bs.BoolSetting('Icy Floor', default=False),
        ]
    ...
    def get_supported_maps(cls, sessiontype: Type[bs.Session]) -> List[str]:
        ...
        for m in ['Lake Frigid', 'Hockey Stadium', 'Football Stadium']:
            # remove maps without bounds if present
            if m in maps:
                maps.remove(m)
        return maps
    ...
    def __init__(self, settings: dict):
        ...
        self._icy_floor = bool(settings.get('Icy Floor', False))
    ...
    def on_transition_in(self) -> None:
        ...
        if getattr(self, '_icy_floor', False):
            try:
                bs.getactivity().map.is_hockey = True
            except Exception:
                pass
...
class SuperSmashElimination(bs.TeamGameActivity[Player2, Team2]):
    ...
    def on_begin(self) -> None:
        ...
        # Safety net: if gloves enabled, equip on any already-alive players.
        if self._boxing_gloves:
            for p in self.players:
                if p.is_alive() and getattr(p, 'actor', None) is not None:
                    try:
