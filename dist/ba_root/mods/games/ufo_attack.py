...
if TYPE_CHECKING:
    from typing import Any, Sequence, Optional, List, Dict, Type
...
class DummyMapForGame(bs.Map):
    ...
    def on_preload(cls) -> Any:
        data: Dict[str, Any] = {'bg_1': bs.gettexture('rampageBGColor'),'bg_2': bs.gettexture('rampageBGColor2'),'bg_model_1': bs.getmesh('rampageBG'),'bg_model_2': bs.getmesh('rampageBG2'),}
        return data
    def __init__(self) -> None:
        super().__init__()
        self.bg1 = bs.newnode('terrain',attrs={'mesh': self.preloaddata['bg_model_1'],'lighting': False,'background': True,'color_texture': self.preloaddata['bg_2']})
        self.bg2 = bs.newnode('terrain',attrs={ 'mesh': self.preloaddata['bg_model_2'], 'lighting': False,'background': True, 'color_texture': self.preloaddata['bg_2']})
...
class DummyMapForGame2(bs.Map):
    ...
    @classmethod
    def on_preload(cls) -> Any:
        # Use the same daylight assets as Tile Lands.
        data: Dict[str, Any] = {
            'bg_1': bs.gettexture('rampageBGColor'),
            'bg_2': bs.gettexture('rampageBGColor2'),
            'bg_model_1': bs.getmesh('rampageBG'),
            'bg_model_2': bs.getmesh('rampageBG2'),
        }
        return data
    def __init__(self) -> None:
        super().__init__()
...
try:
    bs.register_map(DummyMapForGame)
    bs.register_map(DummyMapForGame2)
except Exception:
    try:
...
class UFOAttackGame(bs.TeamGameActivity[Player, Team]):
    ...
    bs.IntSetting('No. of Rows' + u' â†’',max_value=13, min_value=1, default=8, increment=1),
    bs.IntSetting('No. of Columns' + u' â†“', max_value=12, min_value=1, default=6, increment=1)
    ]
    ...
    def __init__(self, settings: dict):
        ...
        def _setting(default: Any, *names: str) -> Any:
            for name in names:
                if name in settings:
                    return settings[name]
            return default

        # Support both clean and mojibake setting labels.
        self.col = max(1, int(_setting(6, 'No. of Columns ↓', 'No. of Columns â†“', 'No. of Columns')))
        self.row = max(1, int(_setting(8, 'No. of Rows →', 'No. of Rows â†’', 'No. of Rows')))
        self.bool1 = bool(_setting(True, 'Enable Run'))
        self.bool2 = bool(_setting(True, 'Enable Jump'))
        self._epic_mode = settings.get('Epic Mode', False)
        self._last_player_death_time: Optional[float] = None
        self._timer: Optional[OnScreenTimer] = None
        self.default_music = (bs.MusicType.EPIC
                              if self._epic_mode else bs.MusicType.SURVIVAL)
        if bool(_setting(False, 'Display Map Area Dimension')):
    ...
    def get_instance_display_string(self) -> str:
    ...
    def on_begin(self) -> None:
        ...
        self._timer.start()
        # Slightly bigger tiles and spacing only on night map.
        tile_scale = 2.4
        tile_step = 3
        try:
            if self.map.getname() == 'Tile Lands Night':
                tile_scale = 2.4
                tile_step = 3
        except Exception:
            pass

        # Keep the grid centered while honoring row/column settings.
        center_x = -1.307783
        center_z = 2.1437
        start_x = center_x - ((self.row - 1) * tile_step) / 2.0
        start_z = center_z - ((self.col - 1) * tile_step) / 2.0

        #bs.timer(5.0, self._check_end_game)
        for r in range(self.col):
            for j in range(self.row):
                tile = TileFloor(
                    pos=(start_x + (j * tile_step), 3.425666, start_z + (r * tile_step)),
                    scale=tile_scale,
                ).autoretain()
    ...
    def end_game(self) -> None:
        try:
            if hasattr(self, 'ufo') and self.ufo is not None:
