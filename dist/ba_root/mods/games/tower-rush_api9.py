...
class BaseRaidGame(ba.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        self._score_sound = bui.getsound('score')
        ...
        def text():
            for team in self.teams:
                if not first_run:
                    if team.role == 'raid':
                        ba.screenmessage(
                                  "Team " + team.name.evaluate() + " has scored " + str(team.stored_score) + " goals in " + str(self.get_s["Countdown Time Each Round"]) + " seconds.",
                                  color=(0.4, 0.6, 1.0)
                                  )
                        break
