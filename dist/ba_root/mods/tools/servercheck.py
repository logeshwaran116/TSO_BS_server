...
ipjoin = {}
recent_join_log_events = {}
_server_boot_time = time.time()
_JOIN_RATE_WINDOW_SECONDS = 15.0
_JOIN_RATE_MAX_BURST = 5
_JOIN_RATE_STARTUP_GRACE_SECONDS = 300.0
_JOIN_WARN_COOLDOWN_SECONDS = 45.0
_DUPLICATE_JOIN_EVENT_SECONDS = 20.0
_JOIN_LOG_DEDUPE_SECONDS = 30.0


def _cleanup_join_tracker(now: float) -> None:
    stale_after = _JOIN_RATE_WINDOW_SECONDS * 10
    for key, data in list(ipjoin.items()):
        if now - data.get("lastJoin", 0.0) > stale_after:
            ipjoin.pop(key, None)

    # Keep join-log dedupe cache bounded.
    for key, ts in list(recent_join_log_events.items()):
        if now - ts > (_JOIN_LOG_DEDUPE_SECONDS * 3):
            recent_join_log_events.pop(key, None)
    ...
    def check(self):
        ...
        for ros in bs.get_game_roster():
            ...
            if ros['account_id'] not in self.players and ros[
                ...
                try:
                    # Suppress duplicate roster join logs for the same account/client
                    # over a short window; roster polling can emit transient duplicates.
                    join_log_key = f'{ros.get("account_id")}|{ros.get("client_id")}'
                    should_emit_join_log = True
                    last_seen = recent_join_log_events.get(join_log_key)
                    if last_seen is not None and now - last_seen < _JOIN_LOG_DEDUPE_SECONDS:
                        should_emit_join_log = False
                    recent_join_log_events[join_log_key] = now
                    if should_emit_join_log:
                        logger.log(
...
def on_player_join_server(pbid, player_data, ip, device_id):
    ...
    if len(ipjoin) > 2000:
        _cleanup_join_tracker(now)

    # Track re-joins by account+ip so users behind the same NAT don't trigger
    # each other's warning, and ignore burst reconnects right after restart.
    join_key = f"{ip}|{pbid}"
    tracker = ipjoin.get(join_key, {})
    lastjoin = tracker.get("lastJoin", 0.0)
    joincount = tracker.get("count", 0)
    lastwarn = tracker.get("lastWarn", 0.0)
    last_client_id = tracker.get("lastClientId")

    # Some clients can trigger duplicate join callbacks while still in the same
    # connection session; ignore those so they do not trip anti-spam warnings.
    if (last_client_id == clid
            and now - lastjoin < _DUPLICATE_JOIN_EVENT_SECONDS):
        ipjoin[join_key] = {
            "lastJoin": now,
            "count": joincount,
            "lastWarn": lastwarn,
    ...
    if now - _server_boot_time >= _JOIN_RATE_STARTUP_GRACE_SECONDS:
        if now - lastjoin < _JOIN_RATE_WINDOW_SECONDS:
            joincount += 1
            if (joincount > _JOIN_RATE_MAX_BURST
                    and now - lastwarn >= _JOIN_WARN_COOLDOWN_SECONDS):
                bs.broadcastmessage("Joining too fast , slow down dude",
                                    color=(1, 0, 1), transient=True,
                                    clients=[clid])
                logger.log(f'{pbid} || warned for joining too fast')
                lastwarn = now
        else:
            joincount = 0
    else:
    ...
    if player_data is not None:  # player data is in serevrdata or in local.json cache
        ...
        if check_ban(ip, device_id, pbid):
            ...
            return
        # Allow new accounts to join: skip minimum account-age restriction
        current_time = datetime.now()
        if pbid not in serverdata.clients:
            # ahh , lets reset if player joining after some long time
...
def save_age(age, pb_id, display_string):
    ...
    thread2.start()
    # Allow new accounts to join: do not kick based on account age
...
def kick_by_pb_id(pb_id, msg):
    # Honor protected role: do not kick protected accounts
    try:
        roles = pdata.get_roles()
        if pb_id in roles.get("protected", {}).get("ids", []):
            return
    except Exception:
        pass
    for ros in bs.get_game_roster():
