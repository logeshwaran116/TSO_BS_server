...
# Mock audioop module before other imports
import sys

class MockAudioop:
    def mul(self, data, width, factor):
        # Return unchanged data since we don't need actual audio processing
        return data

# Add the mock module to sys.modules if audioop is not available
...
def import_games():
    ...
    import sys
    import types
    games_path = _babase.env()['python_directory_user'] + os.sep + "games"
    maps_path = _babase.env()['python_directory_user'] + os.sep + "maps"
    if games_path not in sys.path:
        sys.path.append(games_path)
    if maps_path not in sys.path:
        sys.path.append(maps_path)
    if "games" not in sys.modules:
        games_pkg = types.ModuleType("games")
        games_pkg.__path__ = [games_path]  # type: ignore[attr-defined]
        sys.modules["games"] = games_pkg
    if "maps" not in sys.modules:
        maps_pkg = types.ModuleType("maps")
        maps_pkg.__path__ = [maps_path]  # type: ignore[attr-defined]
        sys.modules["maps"] = maps_pkg

    def _safe_modname(prefix: str, filename: str) -> str:
    ...
    def _import_module_file(pkg_prefix: str, rel_dir: str, filename: str) -> None:
        full_path = os.path.join("ba_root", "mods", rel_dir, filename)
        if not os.path.isfile(full_path):
            return
        ext = os.path.splitext(filename)[1].lower()
        if ext not in (".py", ".so", ".pyd"):
            return
        modname = _safe_modname(pkg_prefix, filename)
        try:
            if modname in sys.modules:
                return
            spec = importlib.util.spec_from_file_location(modname, full_path)
            if spec is None or spec.loader is None:
                logging.warning("Skipping %s (no import spec)", full_path)
                return
            module = importlib.util.module_from_spec(spec)
            sys.modules[modname] = module
            spec.loader.exec_module(module)
        except Exception:
            logging.exception("Failed importing module file: %s", full_path)

    games = os.listdir("ba_root/mods/games")
    for game in games:
        _import_module_file("games", "games", game)

    maps = os.listdir("ba_root/mods/maps")
    for _map in maps:
        _import_module_file("maps", "maps", _map)
...
def on_player_join(self, player) -> None:
    ...
    # Show coinhelp to the joining player instead of a generic welcome
    try:
        account_id = player.get_v1_account_id()
        if account_id:
            # Show onboarding via per-client broadcast messages (only to this player)
            try:
                cid = player.inputdevice.client_id
                def _msg1():
                    bs.broadcastmessage(
                        "Use /coinhelp to see coin commands",
                        color=(0.3, 1.0, 0.6),
                        transient=True,
                        clients=[cid],
                    )
                def _msg2():
                    bs.broadcastmessage(
                        "Use /coins to see your coin details",
                        color=(0.6, 0.9, 1.0),
                        transient=True,
                        clients=[cid],
                    )
                def _msg3():
                    bs.broadcastmessage(
                        "Use our website to get notified when your friend joins",
                        color=(1.0, 0.85, 0.4),
                        transient=True,
                        clients=[cid],
                    )
                bs.apptimer(0.3, _msg1)
                bs.apptimer(2.1, _msg2)
                bs.apptimer(3.9, _msg3)
            except Exception:
                pass
            # Also auto-claim daily reward silently (no spam), if available
            coins.claim_daily_join(account_id, reward=50)
            # Show any queued join messages for this player
            try:
                from playersdata import pdata
                custom = pdata.get_custom()
                msgs = custom.get('join_msgs', {}).get(account_id, [])
                if msgs:
                    for m in msgs[-3:]:  # show last 3 to avoid spam
                        sender = m.get('from_name') or m.get('from') or 'unknown'
                        text = m.get('msg', '')
                        bs.broadcastmessage(f"{text} (msg by {sender})", clients=[player.inputdevice.client_id], transient=True)
            except Exception:
                pass
    except Exception:
        # Avoid impacting join flow on any error
        pass
...
def on_access_check_response(self, data):
    if data is not None:
        addr = data['address']
        port = data['port']
        # If ballistica_web is enabled, set stats URL from main config.json (stats_url field).
        if settings["ballistica_web"]["enable"]:
            try:
                import json
                # Use the server's main config.json path if provided by env, else default to tks/config.json layout.
                cfg_path = _babase.env().get('config_file_path') or os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'config.json')
                with open(cfg_path, encoding='utf-8') as f:
                    cfg = json.load(f)
                stats_url = cfg.get('stats_url') or ''
                if stats_url:
                    bs.set_public_party_stats_url(stats_url)
            except Exception as exc:
                logging.warning("Failed to set stats_url from config.json: %s", exc)
    #https://bombsquad-community.web.app/server-manager/?host={addr}:{port}
